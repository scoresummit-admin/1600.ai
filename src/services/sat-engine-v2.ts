import { SatItem, RoutedItem, AggregatedAnswer, PerformanceMetrics } from '../types/sat';
...
export class SATEngine {
  ...
  private updateMetrics(result: AggregatedAnswer, correctAnswer?: string, startTime?: number): void {
    ...
    if (result.modelVotes.length > 1) {
      this.metrics.escalation_rate = this.metrics.escalation_rate * 0.9 + 0.1; // Exponential moving average
    }
  }
